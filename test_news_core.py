...
class NewsMigrationTests(NewsBase):
    ...
    async def test_news_types_cover_the_three_kinds(self):
        self.assertEqual(set(database.NEWS_TYPES), {"notify", "section", "resource"})
...
class NewsEntityTests(NewsBase):
    async def test_create_all_three_types(self):
...
class NewsFeedTests(NewsBase):
    ...
    async def test_filter_by_type(self):
        ...
        self.assertEqual([n["title"] for n in sections], ["s1"])
